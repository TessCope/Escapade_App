...
exports.createHebergement = async (req, res) => {
  try {
    // en esperant que iduser suive. il faudra changer
    //pour le req de la session quand celui-ci sera implemente
...
exports.updateHebergement = async (req, res) => {
  try {
    const hebergement = await Hebergement.findByIdAndUpdate(req.params.id, req.body, { new: true });
